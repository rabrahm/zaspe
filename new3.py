...
"""
Version of the code created to increase the speed of coputation by determining the continuum normalization after each iteration.
"""
...
if library == 'P':
	...
	gi,gf,dg = 2.0,5.0,0.5
	zi,zf,dz = -1.0,0.5,0.25
	ts = np.arange(ti,tf+1,dt)
	gs = np.arange(gi,gf+.1,dg)
	zs = np.arange(-1.0,0.6,0.25)

Iwav = np.where((mw>wi)&(mw<wf))[0]
mw = mw[Iwav]
...
def get_ccf(wavs,fluxs,ords,vels,ml_v,mh_v,weight):
	for o in ords:
		mflux = scipy.signal.medfilt(fluxs[o],11)
		coef = get_cont(wavs[o],mflux,n=3)
		fluxs[o] = fluxs[o]/np.polyval(coef,wavs[o])
		...
		for i in ords:
			...
			if len(I) > 5:
				...
				if i == ords[0]:
					...
					tmask = np.vstack((tmask,msk))
			I1 = np.where(np.isnan(msk))
			I2 = np.where(np.isnan(fluxs[i]))


		tmask = tmask/np.sum(tmask)
		nflxs = fluxs[ords]/np.sum(fluxs[ords])
...
def get_conti(t,g,z,rot):
    out0,out1,out2,out3,out4,out5,out6 = np.array([]),np.array([]),np.array([]),np.array([]),np.array([]),np.array([]),np.array([])
    mf = get_full_model(t,g,z,rot,RES_POW)
    mwav,mflx,wav,flx = mw.copy(),mf.copy(),sc[0].copy(),sc[3].copy()
    ret_cont = np.ones(sc[0].shape)
    for i in ords:
	...
	mscifO = mscif.copy()
        ...
        ret_cont[i] = np.polyval(coef,sciw)
	mscif = mscif * np.polyval(coef,sciw)
	coef2 = get_cont(sciw,mscif)
	ret_cont[i] = ret_cont[i] / np.polyval(coef2,sciw)

	#coef3 = get_cont(sciw,mscifO)
	#temp_cont = np.polyval(coef3,sciw)
	#out0 = np.hstack((out0,np.zeros(len(sciw))+i))
	#out1 = np.hstack((out1,sciw))
	#out2 = np.hstack((out2,scif))
	#out3 = np.hstack((out3,ret_cont[i]))
	#out4 = np.hstack((out4,temp_cont))
	#out5 = np.hstack((out5,rat))
	#out6 = np.hstack((out6, np.polyval(coef,sciw)))
    #out = np.vstack((out0,out1,out2,out3,out4,out5,out6))
    #np.savetxt('/data/rabrahm/cont_example.txt',out.T)
    #print gfds
    return ret_cont

def get_chis_comp(pars, save=False,plt=False,color='r'):
	t = pars[0]
	g = pars[1]
	z = pars[2]
	rot = pars[3]
	mf = get_full_model(t,g,z,rot,RES_POW)
	#print t,g,z,rot,RES_POW
	#print mf[25000]
	mwav,mflx,wav,flx = mw.copy(),mf.copy(),sc[0].copy(),sc[3].copy()
	res = np.array([])
	Is = 0
	...
	for i in ords:
		...
		#print i, np.sqrt( np.sum(rest**2) / float(len(rest)) )
	"""
	if plt:
...
def get_chis_comp_2(pars, save=False,plt=False,color='r'):
	t = pars[0]
	g = pars[1]
	z = pars[2]
	rot = pars[3]
...
def get_vsini(pars,dplot=False):
	...
	#plot(fwhms,rots,'ro')
	#nfwhms = np.arange(fwhms.min(),fwhms.max(),0.01)
	#plot(nfwhms,interpolate.splev(nfwhms,tck),'k')
	#plot([obfwhm],[rout],'bo')
	#xlabel('FWHM [km/s]',fontsize=18)
	#ylabel('vsin(i) [km/s]',fontsize=18)
	#show()
	#print fghjk
	return rout,NRV
...
def get_rough_pars(spec,RESI=120000.,RV0=0.,guess_vsini=1.0, ncores=6,mask=[],trunc=0,errors=False,use_masks=False,very_rough=False,printing=False,fixG=-1,elim=0.1,zmin=500,nit=15):
	...
	global namespec
	namespec = spec
	global sc,hd,RES_POW,npools,ords,zmask, zmask2, mask_bin, rot_mask, conti
	...
	ml, mh, weight = np.loadtxt(mask_dir,unpack=True)
	ml_v = ToVacuum( ml )
	mh_v = ToVacuum( mh )
	...
	ccf_m = get_ccf(sc[0],sc[3],ords,vels,ml_v,mh_v,weight)
	Im = np.argmin(ccf_m)
	minvel = vels[Im]
	...
	#ords = np.array([6,7])
	for i in ords:
		t1 = np.hstack((t1,sc[0,i]))
		t2 = np.hstack((t2,sc[3,i]))
	t3 = np.vstack((t1,t2))
	np.savetxt('data_temp_old.txt',t3)

	xis = get_chis_comp([4600,4.6,0.0,1.0],plt=True)
	print xis
	print fdxs
	"""
	#get_chis_comp([6350,3.4,-0.34,10.75],plt=True,color='r')
	#print gfdx
	#get_chis_comp([6509,4.14,-0.02,10.6],plt=True,color='g')
	#show()
	#print gfd
	...
	while ik < nit:
		...
		if ik == 0:
			...
			final_chis = np.array((p.map(get_chis_comp_2, models)))
		p.terminate()
		...
		past_curr = curr
		dplot=False
		conti = get_conti(curt,curg,curz,curr)
		...
		if not errors and use_masks:
			...
			#ZO,ZI,ZF,BZO,BZI,BZF = the_good_zones(ZO,ZI,ZF,[curt,curg,curz,curr,RVTOT],th=3.0,zmin=zmin,limit=elim)
			ZO,ZI,ZF,BZO,BZI,BZF = the_good_zones_2(ZO,ZI,ZF,[curt,curg,curz,curr,RVTOT],th=3.0,zmin=zmin,limit=elim)
...
def get_zones(pars):
	...
	#lim6000, lim5000 = 0.13,0.2
	lim_c = np.polyfit([5000.,6000.],[lim5000,lim6000],1)
	#lim = 0.05
	#lim_c = np.array([ -5e-05,   4.0e-01])
	...
	II = np.where(lim<lim6000)[0]
	lim[II] = lim6000
	II = np.where(lim>lim5000)[0]
	lim[II] = lim5000

	#ZJI = np.where((mw>5355)&(mw<5380))[0]
	#for i in ZJI:
	#    print mw[i],diff[i],lim[i]
	#plot(mw[ZJI],diff[ZJI])
	#plot(mw[ZJI],lim[ZJI],'r')
	#show()
	#print gfds
	...
	#plot(mw,diff)
	#plot(mw,lim,linewidth=2.)
	#xlabel(r'Wavelength [$\AA$]',fontsize=18)
	#ylabel(r'$\Delta S_{\lambda} (\theta)$',fontsize=18)
	#print len(miw)
	#show()
	#print gfds
...
def the_good_zones_2(ZO,ZI,ZF,pars, th=3.0,zmin=400,limit=0.001):
	...
	for i in ords:
		...
		tmodf[i] = modf
		tscif[i] = scif
	#	ZIJ = np.where((sciw>5355) & (sciw<5380))[0]
	#	if len(ZIJ)>0:
	#		for ii in ZIJ:
	#			print sciw[ii],scif[ii]
	#print gfds
	...
	for i in range(len(ZI)):
		...
		factors = np.append(factors,f)
	#print len(ZO)
	IB1 = np.where(deltzones<0.05)[0]
	IB2 = np.where(deltzones>1.0)[0]
	IB3 = np.where(factors>2)[0]
	IB4 = np.where(factors<0.5)[0]
	#IB3 = np.where(lenzones<5)[0]
	IB = np.unique(np.hstack((IB1,IB2,IB3,IB4)))
	IG = np.where((deltzones>=0.05)&(deltzones<=1.0)&(factors<=2)&(factors>=.5))[0]
	BZO = np.hstack((BZO,ZO[IB]))
	BZI = np.hstack((BZI,ZI[IB]))
	BZF = np.hstack((BZF,ZF[IB]))
	ZO  = ZO[IG]
...
def get_precise_parameters(spec,rough_parameters,RESI=120000.,ncores=14,trunc=0,fixG=-1,elim=0.1,zmin=500,nsim=50,efixG=-1):
	...
	ZO,ZI,ZF = get_zetas(zi,zf)
	ZO,ZI,ZF,BZO,BZI,BZF = the_good_zones_2(ZO,ZI,ZF,rough_parameters,th=3.0,zmin=zmin,limit=elim)
	#for i in range(len(ZO)):
	#	if sc[0,ZO[i],ZI[i]] > 5355 and sc[0,ZO[i],ZI[i]] < 5380: 
	#		print sc[0,ZO[i],ZI[i]],sc[0,ZO[i],ZF[i]]
	#print gfd
	#mf = get_full_model(rt,rg,rz,rr,60000.)
	#I = np.where((mw>5355) & (mw<5380))[0]
	#for i in I:
	#	print mw[i],mf[i]
	#print gfds
	#print len(ZO)
